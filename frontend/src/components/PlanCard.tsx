...
export function PlanCard({
  ...
  currentPermission: "inspect" | "approve" | "unrestricted";
}) {
  const editable = plan.state === "AWAITING_CONFIRMATION";
  const permissionLabel = plan.execution_permission_mode === "approve"
    ? "逐次批准"
    : plan.execution_permission_mode === "unrestricted"
      ? "受控直接执行"
      : "只读检查";
  const currentPermissionLabel = currentPermission === "approve"
    ? "逐次批准"
    : currentPermission === "unrestricted"
      ? "受控直接执行"
      : "只读检查";
  const resumeChangesPermission = plan.state === "PAUSED"
    && plan.execution_permission_mode !== currentPermission;
  return (
    <section className="plan-card" aria-label="执行计划">
      ...
      <p className="plan-card-permission">
        {editable ? `确认时执行权限：${permissionLabel}` : `本次执行权限：${permissionLabel}`}
        {editable && "（计划生成阶段始终只读）"}
      </p>
      ...
      <div className="plan-card-actions">
        ...
        {plan.state === "PAUSED" && <button type="button" className="primary" onClick={onResume}><RotateCcw size={14} />{resumeChangesPermission ? `以${currentPermissionLabel}恢复` : "恢复计划"}</button>}
